...
can be either a char* or wchar_t* buffer.
  ...
  @returns 0 on success, -1 on failure. Will fail if the buffer is either
invalid or not large enough to hold the converted value.*/
...
/**Determines whether a character is a space, tab, or newline. Also includes
double-width and no break spaces.
...
/**Converts string in hex format to int. Default figures out how much of the
   string is a valid hex string, but passing a value to the second parameter
   overrides this and allows you to indicate how much of the string to try to
   convert.
    @param hexStr The string to convert.
    @length How much of the string to analyze. The value -1 (the default) will
   tell the function to read until there are no more valid hexadecimal digits.
...
/**@returns The number of characters in the string pointed to by str, not
   including the terminating '\0' character, but at most maxlen. In doing this,
   strnlen looks only at the first maxlen characters at str and never beyond
   str+maxlen. This function should be used for input that may not be NULL
   terminated.
...
/**Searches for substring in a larger string (case-insensitively), limiting the
search to a specified number of characters.*/
template <typename T>
            ...
            if (iswdigit(ch1) && iswdigit(ch2))
                {
                    ...
                    if (firstDouble < secondDouble)
                        ...
                        {
                            // if this was the end of both strings then they
                            // are equal
                            if (*firstEnd == 0 && *secondEnd == 0)
                                {
                                    return 0;
                                }
                            /*the first string is done, but there is more to
                            the second string after the number, so first is
                            smaller*/
                            else if (*firstEnd == 0)
                                {
                                    return -1;
                                }
                            /*the second string is done, but there is more to
                            the first string after the number, so first is
                            bigger*/
                            else if (*secondEnd == 0)
                                {
                                    return 1;
                                }
                            // there is more to both of them, so move the
                            // counter and move on
                            else
                                {
                                    first_string_index =
                                        (firstEnd - first_string);
                                    second_string_index =
                                        (secondEnd - second_string);
                                    continue;
                                }
                        }
                }
...
/// Indicates whether a larger strings ends with the specified suffix. Lengths
/// are provided by the caller for efficiency. This function is case sensitive.
template <typename T>
...
/**Searches for a matching tag, skipping an extra open/close pairs of symbols
in between.
...
@returns A pointer to where the closing tag is, or NULL if one can't be
found.*/
template <typename T>
    ...
    while (*string)
        {
            ...
            if (string[0] == openSymbolFirstCharacter &&
                string_util::strncmp(string, openSymbol, openSymbolLength) ==
                    0)
                {
                    ++open_stack;
                ...
                string_util::strncmp(string, closeSymbol, closeSymbolLength) ==
                    0)
                {
                    if (open_stack == 0)
                        {
...
/**Searches for a single character in a string for n number of characters.
    Size argument should be less than or equal to the length of the string
   being searched.
    @param string The string to search in.
    @param ch The character to search for.
    @param numberOfCharacters The number of characters to search through in the
   string.
    @returns A pointer in the string where the character was found, or NULL if
   not found.*/
template <typename T>
...
/**Searches for a single character from a sequence in a string for n number of
   characters.
    ...
    @returns The index into the string that the character was found. Returns
   the length of the string if not found.*/
template <typename T>
    ...
    for (i = 0; i < stringToSearchLength; ++i)
        {
            ...
            actual position that we stopped on which would be a NULL
            termintor.*/
...
/**Searches for a single character not from a sequence in a string in reverse.
    ...
    @param offset Where to begin the search. If -1, then the reverse search
   will begin at the end of the string.
    @returns The position of where the last non-matching character is at, or -1
    ...
    while (offset != std::basic_string<T>::npos)
        {
            ...
            // If the whole search string wasn't scanned, then there was a
            // match. Move on.
            if (search[i] != 0)
                {
                    if (offset == 0)
...
/**Searches for the last instance of a character in a string in reverse.
    ...
    @param ch The character to search for.
    @param offset The offset in the string to begin the search from. The
   default (-1) will begin the search at the end of the string.
...
/*Searches for the first occurrence that is not a character from a sequence in
   ...
   with.
    ...
    the length of the string if nothing was found that couldn't match the
   search string.*/
template <typename T>
    ...
    const T * searchString, const size_t searchStringLength)
{
    ...
    for (i = 0; i < stringToSearchLength; ++i)
        {
            /*If string being searched is shorter
            than the size argument then return string length to indicate
            failure to find anything. Although this isn't really the index that
            we stopped in, this will tell the caller that the search failed.*/
            if (stringToSearch[i] == 0)
                {
                    NON_UNIT_TEST_ASSERT(
                        0 &&
...
public:
    ...
    /// This is useful for seeing if there are any delimiters at all when first
    /// loading the string.
    ...
    /// Note that empty tokens can be returned if there is proceeding or
    ...
    for (unsigned int i = 0; i < Text.length(); ++i)
        {
            ...
            // this is extra space right after another--get rid of it
                ...
                is_space(static_cast<wchar_t>(Text[i])) && alreadyHasSpace)
                {
    ...
    for (unsigned int i = 0; i < Text.length(); ++i)
        {
            ...
            // we are starting another word--reset
                         ...
                         static_cast<wchar_t>(Text[i]), 10, 13))
                {
...
/**Converts strings to double values, but also takes into account ranges
