...
\def CASSERT(expression)
    Validates that an expression is true at compile time. If the expression is
false then compilation will fail. \def NON_UNIT_TEST_ASSERT(expression) If unit
test symbol (__UNITTEST) is defined then does nothing; otherwise asserts. This
is useful for suppressing asserts when unit testing. \def
DUMP_TO_FILE(stream,file) Prints data stream to a specified file.*/
/** @} */
